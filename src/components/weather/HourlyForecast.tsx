...
export function HourlyForecast({ hourly, tempUnit, className = '' }: Props) {
  ...
  return (
    <section className={`flex w-full flex-col items-center ${className}`}>
      <div className="mb-4 flex items-center justify-center gap-3 px-1">
        <h2 className="text-base font-semibold tracking-[0.2em] text-white/65">HOURLY</h2>
        <span className="text-sm text-white/40">未来 12 小时</span>
      </div>
        ...
        <div className="no-scrollbar flex gap-3 overflow-x-auto pb-1">
          {hours.map((h, i) => {
            ...
            return (
              <div
                key={i}
                className={`flex min-w-[70px] flex-col items-center justify-center gap-2 rounded-2xl px-2 py-4 ${
                  now ? 'bg-white/[0.025]' : 'bg-transparent'
                }`}
              >
                <span className={`text-sm tabular-nums ${now ? 'text-white/85' : 'text-white/55'}`}>
                  {now ? 'Now' : hourLabel(h.time)}
                </span>
                <WeatherGlyph condition={h.condition} size={36} className="text-white/90" />
